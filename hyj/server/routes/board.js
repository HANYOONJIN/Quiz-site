...
//패키지 리스트 5개 가져오기
router.get('/packageList5', (req, res) =>{
	conn.query("SELECT * FROM package ORDER BY num DESC LIMIT 5", (err, results) => {
		if(err) {
			return res.send(err);
		}else {
			return res.json({
...
//퀴즈 리스트 5개 가져오기
router.get('/quizList5', (req, res) =>{
	conn.query("SELECT * FROM quiz ORDER BY num DESC LIMIT 5", (err, results) => {
		if(err) {
			return res.send(err);
		}else {
			return res.json({
...
router.post('/searchPackage', (req, res) =>{
	...
	let sql = "SELECT * FROM package where title REGEXP ? OR content REGEXP ? ORDER BY num DESC";
...
router.post("/quizInsert", (req, res) => {
	...
	*/
	...
	let sql = "INSERT INTO quiz (quizname, optionlist, tag, answer, score, id, level)" 
	+ " SELECT ?,?,?,?,?,?,? FROM DUAL"
	+	" WHERE NOT EXISTS (SELECT * FROM quiz WHERE quizname=? AND optionlist=? AND tag=? AND answer=? AND score=? AND level=?) LIMIT 1";
	let sql2 = "SELECT num FROM quiz WHERE quizname=? AND optionlist=? AND tag=? AND answer=? AND score=? AND level=?";
	let post = [quizname, optionlist, tag, answer, score, id, level,quizname, optionlist, tag, answer, score, level];
	let post2 = [quizname, optionlist, tag, answer, score, level];
	conn.query(sql, post, (err, results, fields) => {
		if (err) {
			console.log(err);
			return res.status(400).json({
			...
			console.log('퀴즈등록결과:'+results.insertId);
			if(results.insertId==0){
				conn.query(sql2, post2, (err2, results2, fields2) => {
					return res.status(200).json({
						num : results2[0].num
					});
				})
...
//퀴즈 검색
router.post('/searchQuiz', (req, res) =>{
	const content = req.body.content;

	let sql = "SELECT * FROM quiz where quizname REGEXP ? ORDER BY num DESC";
	let post = [content, content];
	conn.query(sql, post, (err, results, fields) => {
		if(err) {
			return res.send(err);
